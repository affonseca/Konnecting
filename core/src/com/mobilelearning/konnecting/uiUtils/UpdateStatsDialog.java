...
import com.mobilelearning.konnecting.serviceHandling.services.GetLeaderboardScoresService;
import com.mobilelearning.konnecting.serviceHandling.services.UpdateStatsService;
...
public class UpdateStatsDialog implements UpdateStatsHandler, GetLeaderboardScoresHandler {
    ...
    private int lastLeaderboardTypeID;
    ...
    public void getLeaderboardScores(int lastLeaderboardTypeID){
        screen.startLoadingAnimation(false);
        GetLeaderboardScoresService service = new GetLeaderboardScoresService(this);

        this.lastLeaderboardTypeID = lastLeaderboardTypeID;
        service.requestLeaderboardScores(
                Long.parseLong(SavedData.getCurrentClassID()),
                lastLeaderboardTypeID,
                SavedData.getStats()
        );
    }
    ...
    private void showDialog(String errorMessage, ButtonType[] buttons){
            ...
            protected void result(Object object) {
                if (((String) object).contains("continue")) {
                    ...
                    if(callback instanceof GetLeaderboardScoresDialogCallback)
                        getLeaderboardScores(lastLeaderboardTypeID);
                    else
                        updateStats();
                }
                else { //if goBack
        ...
        out.getCell(out.getContentTable()).expandY();
        out.getContentTable().center().padTop(12f);

        out.getCell(out.getButtonTable());
        out.getButtonTable().center().padBottom(34f).defaults().padTop(11f);
